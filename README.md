...
**v4.0 核心升级**：从「固定参数推演」进化到「自适应智能推演」——新增 Phase 0 智能配置、自动收敛检测、多模型博弈、结构化记忆、ReACT 报告生成。
...
| **Layer 2** | 标准 | sessions_spawn 并行 Agent + 智能配置 + 自动收敛 | 5-8 角色、中等复杂度 |
...
| 文件 | 内容 |
|------|------|
| `SKILL.md` | 主技能 — 三层架构 + 12维矩阵 + v4.0 执行协议 |
...
| `report-agent.md` | **v4.0** ReportAgent — ReACT 模式报告生成（规划→逐章节→反思→合并） |
| `config-generator.md` | **v4.0 新增** — Phase 0 智能配置生成（分步生成 + JSON 修复 + 规则回退） |
| `deliberation-memory.md` | **v4.0 新增** — 结构化记忆（JSONL 日志 + 收敛检测 + 跨轮注入） |
